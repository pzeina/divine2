...
void dve_compiler::output_dependency_comment( ext_transition_t &ext_transition, bool condition )
{
    // only for ltsmin
    if (!ltsmin)
        return;
    ...
    switch (expr.get_operator())
    {
        ...
        case T_SQUARE_BRACKETS:
            if ((*expr.left()).get_operator() == T_NAT)
            {
                ...
                if (dep == ext_transition.sv_may_write || dep == ext_transition.sv_read || dep == ext_transition.sv_actions_read) {
                    mark_dependency(expr.get_ident_gid(), state_creator_t::VARIABLE, -1, dep);
                }
                ...
                if ((*expr.left()).get_operator() == T_ASSIGNMENT) {
                    ...
                    analyse_expression(*expr.left(), ext_transition, ext_transition.sv_actions_read);
        ...
        case T_BOOL_AND: case T_BOOL_OR:
            if (dep == ext_transition.sv_read || dep == ext_transition.sv_actions_read) {
                analyse_expression( *expr.left(), ext_transition, dep );
        ...
        case T_ASSIGNMENT:
            ...
            analyse_expression( *expr.right(), ext_transition, ext_transition.sv_actions_read );
            break;
        case T_DOT:
...
{
    output_dependency_comment(*et, true);
...
void dve_compiler::transition_effect( ext_transition_t *et, std::string in, std::string out )
{
    ...
    output_dependency_comment(*et, false);
}
                       ...
                       iter_process_transition_map->first))
                {
                        ...
                        iter_ext_transition_vector++)
                    {
                            ...
                            get_commited(iter_ext_transition_vector->second->get_state1_lid()) )
                        {
                            ...
                            new_label();
                            if (condition) { // generate get_successor
                                block_begin();
                                transition_guard( &*iter_ext_transition_vector, in );
                                line("states_emitted = get_action(model, t, in, callback, arg);");
                                block_end();
...
void dve_compiler::gen_transition_info()
{
    ...
    for(size_int_t i = 0; i < transitions.size(); i++) {
        ...
        for(size_int_t j = 0; j < transitions.size(); j++) {
            ...
            if (!current2.buchi) {
                gs.insert(gs.begin(), gs2.begin(), gs2.end());
            }
        }
    }
